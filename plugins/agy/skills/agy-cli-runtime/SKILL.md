...
- You may use the `agy-prompting` skill to rewrite the user's request into a tighter Codex prompt before the single `task` call.
...
Safety rules:
- Default to write-capable Codex work in `agy:agy-rescue` unless the user explicitly asks for read-only behavior.
